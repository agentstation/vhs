		...
		if g.options.Debug {
			...
			for _, lineColors := range state.LineColors {
				for _, style := range lineColors {
					if style.BgColor != "" && style.BgColor != "<nil>" && style.BgColor != "null" {
						bgCount++
					}
				}
			}
				...
				if afterCursor != "" {
					...
					for i := 0; i < len(afterRunes); {
						...
						if hasColors && y < len(state.LineColors) && charPos < len(state.LineColors[y]) {
							...
							if style.FgColor != "" && style.FgColor != "<nil>" {
						...
						for i < len(afterRunes) {
							...
							if hasColors && y < len(state.LineColors) && nextCharPos < len(state.LineColors[y]) {
								...
								if nextStyle.FgColor != "" && nextStyle.FgColor != "<nil>" {
								...
								if nextStyle.Bold {
									nextStyleStr += "font-weight:bold;"
								}
								if nextStyle.Italic {
									nextStyleStr += "font-style:italic;"
								}
								if nextStyle.Underline {
									nextStyleStr += "text-decoration:underline;"
								}
							}
...
// shouldUsePromptClass checks if a color should use the prompt class.
func (g *SVGGenerator) shouldUsePromptClass(color string) bool {
	if !g.options.OptimizeSize || color == "" {
		return false
	}
	// Check if it matches the bright blue color (common for prompts)
	theme := g.options.Theme
	return color == theme.BrightBlue || color == "#5a56e0"
}
...
func (g *SVGGenerator) renderTextSegment(sb *strings.Builder, text string, lineIndex, startChar, endChar int, hasColors bool, lineColors [][]CharStyle) {
	runes := []rune(text)
	x := 0
...
// nl returns a newline or empty string based on optimization settings.
func (g *SVGGenerator) nl() string {
	if g.options.OptimizeSize {
		return ""
	}
	return "\n"
	...
	if !lineColorsJSON.Nil() {
		...
		for _, line := range lines {
			...
			for _, charData := range chars {
				...
				if !fgColorVal.Nil() && fgColorVal.Str() != "<nil>" {
					fgColor = fgColorVal.Str()
				}
				if !bgColorVal.Nil() && bgColorVal.Str() != "<nil>" {
...
func buildSVGFontFamily(fontFamily string) string {
	...
	for _, font := range fonts {
		...
		if font == "monospace" || font == "ui-monospace" || font == "sans-serif" || font == "serif" {
	...
	if !hasMonospace {
		fontList = append(fontList, "monospace")
	}
