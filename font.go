...
// getTextYPosition calculates the Y position for centered text in the window bar.
func getTextYPosition(barHeight int) int {
	// Center vertically in the bar with a slight offset
	// The font baseline is typically about 2/3 of the way down
	// For Face7x13, the height is 13 pixels
	fontHeight := 13
	return barHeight/2 + fontHeight/3
}
...
func (fl *FontLoader) loadFontFromFamily(fontFamily string, fontSize float64) (font.Face, error) {
	...
	for _, fontName := range fonts {
		...
		if fontName == "monospace" || fontName == "ui-monospace" {
			continue
		}
