...
func TestSVGGenerator_UtilityFunctions(t *testing.T) {
	t.Run("shouldUsePromptClass", func(t *testing.T) {
		// Test with optimization disabled
		opts := createTestSVGConfig()
		opts.OptimizeSize = false
		gen := NewSVGGenerator(opts)

		if gen.shouldUsePromptClass("#5a56e0") {
			t.Error("shouldUsePromptClass should return false when optimization is disabled")
		}
		...
		// Test empty color
		if gen.shouldUsePromptClass("") {
			t.Error("shouldUsePromptClass should return false for empty color")
		}

		// Test prompt color
		if !gen.shouldUsePromptClass("#5a56e0") {
			t.Error("shouldUsePromptClass should return true for prompt color #5a56e0")
		}

		// Test bright blue theme color
		opts.Theme.BrightBlue = "#00aaff"
		gen = NewSVGGenerator(opts)
		if !gen.shouldUsePromptClass("#00aaff") {
			t.Error("shouldUsePromptClass should return true for theme's bright blue color")
		}

		// Test non-prompt color
		if gen.shouldUsePromptClass("#ff0000") {
			t.Error("shouldUsePromptClass should return false for non-prompt color")
		}
	})
	...
	t.Run("nl function", func(t *testing.T) {
		// Test with optimization disabled
		opts := createTestSVGConfig()
		opts.OptimizeSize = false
		gen := NewSVGGenerator(opts)
		...
		if gen.nl() != "\n" {
			t.Error("nl() should return newline when optimization is disabled")
		}
		...
		if gen.nl() != "" {
			t.Error("nl() should return empty string when optimization is enabled")
		}
	})
}
	...
	t.Run("handles file write error gracefully", func(t *testing.T) {
		vhs := &VHS{
			Options: &Options{
				...
				Video: VideoOptions{
					Output: VideoOutputs{
						SVG: "/invalid/path/that/does/not/exist/output.svg",
					},
					Style: DefaultStyleOptions(),
				},
			},
