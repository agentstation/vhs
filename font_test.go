...
// Test text Y position calculations
func TestGetTextYPosition(t *testing.T) {
	testCases := []struct {
		barHeight int
		expected  int
	}{
		{30, 19}, // 30/2 + 13/3 = 15 + 4 = 19
		{40, 24}, // 40/2 + 13/3 = 20 + 4 = 24
		{20, 14}, // 20/2 + 13/3 = 10 + 4 = 14
	}
	
	for _, tc := range testCases {
		y := getTextYPosition(tc.barHeight)
		if y != tc.expected {
			t.Errorf("getTextYPosition(%d) = %d, expected %d", tc.barHeight, y, tc.expected)
		}
	}
}
